...
const TeammateFinderPage: React.FC = () => {
    ...
    const fetchRegistrations = async (project: string, bonus: string, deadline: string) => {
        try {
            let url = `${BACKEND_URL}/api/project-registrations/search?projectName=${project}`;
    ...
    const handleCreateSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            await axios.post(`${BACKEND_URL}/api/project-registrations`, formData, config);
    ...
    const handleDelete = async (regId: number) => {
        ...
        try {
            await axios.delete(`${BACKEND_URL}/api/project-registrations/${regId}`, config);
            fetchRegistrations(appliedProject, appliedBonus, appliedDeadline);
        } catch (err) {
